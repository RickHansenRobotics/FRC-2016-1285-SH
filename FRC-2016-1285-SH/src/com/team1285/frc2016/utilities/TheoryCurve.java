...
public class TheoryCurve {
	...
	public Vector findPoints() {
		...
		for (double x = 0; x <= 1.05; x += 0.05) {
			...
			points.addElement(useFunctionX(xValues[0], xValues[1], xValues[2], xValues[3], x) + ", "
					+ useFunctionY(yValues[0], yValues[1], yValues[2], yValues[3], x));
		}
	...
	public void calcPoints() {
		for (int x = 0; x < xDelta.length; x++) {
			xDelta[x] = (Double.parseDouble(xPoints.elementAt(x + 1).toString())
					- Double.parseDouble(xPoints.elementAt(x).toString()));
			yDelta[x] = (Double.parseDouble(yPoints.elementAt(x + 1).toString())
					- Double.parseDouble(yPoints.elementAt(x).toString()));
